...
const HeaderComponent = ({ongoing, pending, completed, cancel}) => {
  ...
  const Task = ({item}) =>{
    return(
    <Pressable style={{width:'45%', backgroundColor:item.color, padding:10,  borderRadius:10, margin:10,  borderRadius: 10,}} >
      {item.icon}
        <View style={{flexDirection:'row', justifyContent:'space-between', alignItems:'center', marginTop:30 }}>
       <View>
       <Text style={{color:AppColors.WHITE, fontSize:14, fontWeight:'600'}}>{item.title}</Text>
        <Text style={{color:AppColors.WHITE, fontSize:16, fontWeight:'600', marginTop:5}}>{item.count} Task </Text>
  ...
  return (
    <View >
      ...
      />
      <View>
        <Text style={{fontSize:18, fontWeight:'500', margin:10, marginHorizontal:20}}>All Task</Text>
      </View>
